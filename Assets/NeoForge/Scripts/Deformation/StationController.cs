...
namespace NeoForge.Deformation
{
    ...
    {
        [Tooltip(("Event that is called when the station is changed"))] [SerializeField]
        private UnityEvent<Station> _stationChanged;
        ...
        [Tooltip("The shared state that holds the current station of the player")] [SerializeField]
        private SharedState _currentStation;

        [Tooltip("The mesh similarity calculator that is used to compare the current mesh to the target mesh")]
        [SerializeField]
        private MeshSimilarityCalculator _meshSimilarityCalculator;

        [Header("UI Screens")]
        [Tooltip("The main UI elements that are displayed when the player is in a specific station")]
        [SerializeField]
        private SerializedDictionary<Station, GameObject> _uiElements;

        [Tooltip("A UI screen that is displayed after the user selects a part in the main heating UI")] [SerializeField]
        private GameObject _heatingUI;

        [Tooltip("A UI screen that prompts the user to return the part to the heating station")] [SerializeField]
        private GameObject _returnUI;

        [Tooltip("A UI screen that contains the heating and cooling buttons and a temperature dislpay")]
        [SerializeField]
        private GameObject _temperatureUI;

        [Header("Part Positions")] [Tooltip("The transform for the part at each station")] [SerializeField]
        private SerializedDictionary<Station, Transform> _partPositions;

        [Tooltip("The position of the part when it is in the water")] [SerializeField]
        private Transform _inWaterPosition;

        [Tooltip("The initial position of the part in the heating station")] [SerializeField]
        private Transform _initialFurnacePosition;

        [Header("Temperature Displays")]
        [Tooltip("The text display that shows the temperature of the part")]
        [SerializeField]
        private TMPro.TMP_Text _temperatureDisplay;

        [Tooltip("The button that toggles the part between heating and cooling")] [SerializeField]
        private TMPro.TMP_Text _heatingCoolingButton;
